    ...
    TGenericExecutorThread::TProcessingResult TGenericExecutorThread::Execute(TMailbox* mailbox, bool isTailExecution) {
        ...
        for (; Ctx.ExecutedEvents < Ctx.EventsPerMailbox; ++Ctx.ExecutedEvents) {
            if (TAutoPtr<IEventHandle> evExt = mailbox->Pop()) {
                // TODO: mailbox->ProcessEvents(mailbox);
                ...
                if (actor = mailbox->FindActor(recipient.LocalId())) {
                    ...
                    if (firstEvent) {
                        double usec = Ctx.AddActivationStats(mailbox->ScheduleMoment.load(std::memory_order_relaxed), hpprev);
                        if (usec > 500) {
                ...
                if (TlsThreadContext->CapturedType == ESendingType::Tail) {
                    mailbox->ScheduleMoment.store(hpnow, std::memory_order_relaxed);
        ...
        if (mailbox->IsEmpty() && drained) {
            ...
            mailbox->Unlock(Ctx.Executor, RevolvingWriteCounter);
        }
